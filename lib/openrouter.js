...
async function analyzeSeedWithGroq(imageUrl, manualName = null) {
  const apiKey = process.env.GROQ_API_KEY;
  const model = process.env.GROQ_MODEL || 'llama-3.1-8b-instant';
  
  console.log(`[Groq] Starting analysis using model "${model}"${manualName ? ` for manual name "${manualName}"` : ''}.`);
  ...
  const promptText = SEED_ANALYSIS_PROMPT + (manualName ? `\n\nCRITICAL DIRECTIVE: The user has manually specified that this seed is: "${manualName}". Please bypass standard visual identification and construct the detailed plant result database info specifically for "${manualName}" based on your expert botanical database. Provide accurate planting and care guidelines for this plant.` : '');
...
export async function analyzeSeed(imageUrl, manualName = null) {
  const useGroq = process.env.USE_GROQ === 'true';
  if (useGroq) {
    const model = process.env.GROQ_MODEL || 'llama-3.1-8b-instant';
    console.log(`[Seed Analysis] Routing request to GROQ using model: "${model}"`);
    return analyzeSeedWithGroq(imageUrl, manualName);
  }
            ...
            messages: [
              {
                ...
                content: [
                  {
                    type: 'text',
                    text: SEED_ANALYSIS_PROMPT + (manualName ? `\n\nCRITICAL DIRECTIVE: The user has manually specified that this seed is: "${manualName}". Please bypass standard visual identification and construct the detailed plant result database info specifically for "${manualName}" based on your expert botanical database. Provide accurate planting and care guidelines for this plant.` : ''),
                  },
                  {
                    type: 'image_url',
                    image_url: {
